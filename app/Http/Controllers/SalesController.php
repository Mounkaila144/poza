...
class SalesController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $client_id = $request->client_id ?? Sale::DEFAULT_CLIENT_ID;
        ...
        \DB::transaction(function () use ($request, $client_id) {
            ...
            foreach ($data as $key => $value) {
                $sale_unit_id = $value['sale_unit_id']?? Sale::DEFAULT_SALES_UNIT_ID;
                $unit = Unit::where('id', $sale_unit_id)
                    ->first();
                $orderDetails[] = [
                    'date'         => $request->date,
                    'sale_id'      => $order->id,
                ...
                try {
                    ...
                    if($request['amount'] > 0 && $request->payment['status'] != 'pending'){
                        if ($request->payment['Reglement'] == 'credit card') {
                            ...
                            $PaymentWithCreditCard = PaymentWithCreditCard::where('customer_id', $client_id)->first();
                            ...
                            $PaymentCard['customer_id'] = $client_id;
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::transaction(function () use ($request, $id) {
            ...
            if (SaleReturn::where('sale_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                $current_Sale->update([
                    'date'         => $request['date'],
                    'client_id'    => $request['client_id']?? $current_Sale->client_id?? Sale::DEFAULT_CLIENT_ID,
