...
class DashboardController extends Controller
{
    ...
    public function Top_Products_Year($warehouse_id, $array_warehouses_id)
    {
        ...
        $products = PurchaseDetail::join('purchases', 'purchase_details.purchase_id', '=', 'purchases.id')
...
//        // top purchase product this month
//        $products = PurchaseDetail::join('purchases', 'purchase_details.purchase_id', '=', 'purchases.id')
//            ->join('products', 'purchase_details.product_id', '=', 'products.id')
//            ->whereBetween('purchases.date', [
//                Carbon::now()->startOfMonth(),
...
//            ->get();
        ...
        $pending_purchases = Purchase::where('statut', 'pending')
            ->with('provider', 'warehouse') // Chargement des relations
            ->whereNull('deleted_at') // Utilisation de whereNull pour vérifier si deleted_at est NULL
            ->where(function ($query) use ($view_records) {
                if (!$view_records) {
                    // Si l'utilisateur ne peut voir que ses propres enregistrements
                    $query->where('user_id', Auth::user()->id);
                }
            })
            ->where(function ($query) use ($warehouse_id, $array_warehouses_id) {
                if ($warehouse_id !== 0) {
                    // Si un entrepôt spécifique est sélectionné
                    $query->where('warehouse_id', $warehouse_id);
                } else {
                    // Sinon, filtre par plusieurs entrepôts
                    $query->whereIn('warehouse_id', $array_warehouses_id);
                }
            })
            ->take(5) // Limitation à 5 résultats
            ->get();

        // $pending_purchases = Purchase::where('', '')
        ...
        // Stock Alerts
        $product_warehouse_data = product_warehouse::with('warehouse', 'product', 'product.unit', 'product.category', 'product.supplier', 'product.brand', 'productVariant')
        ...
        if ($product_warehouse_data->isNotEmpty()) {
            ...
            foreach ($product_warehouse_data as $product_warehouse) {
                if ($product_warehouse['product'] && ($product_warehouse->qte <= $product_warehouse['product']->stock_alert)) {
                    if ($product_warehouse->productVariant !== null) {
                    ...
                    if ($product_warehouse['warehouse']) {
        ...
        return response()->json([
            ...
            'last_adjustments' => $last_adjustment,
