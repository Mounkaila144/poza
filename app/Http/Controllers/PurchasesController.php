...
class PurchasesController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($data as $key => $value) {
                $unit = Unit::where('id', $value['purchase_unit_id'] ?? Purchase::DEFAULT_UNIT_ID)->first();
                $orderDetails[] = [
                    'purchase_id' => $order->id,
                    'quantity' => $value['quantity'],
                    'cost' => $value['Unit_cost'],
                    'purchase_unit_id' =>  $value['purchase_unit_id'] ?? Purchase::DEFAULT_UNIT_ID,
                ...
                if ($order->statut == "received") {
                    if ($value['product_variant_id'] !== null) {
                        ...
                        if ($unit && $product_warehouse) {
                            if ($unit->operator == '/') {
                                $product_warehouse->qte += $value['quantity'] / ($unit->operator_value ?? 1);
                            } else {
                                $product_warehouse->qte += $value['quantity'] * ($unit->operator_value ?? 1);
                            }
            ...
            if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                foreach ($new_purchase_details as $key => $prod_detail) {
                    ...
                    if ($prod_detail['no_unit'] !== 0) {
                        $unit_prod = Unit::where('id', $prod_detail['purchase_unit_id'] ?? Purchase::DEFAULT_UNIT_ID)->first();
                            ...
                            if ($prod_detail['product_variant_id'] !== null) {
                                ...
                                if ($unit_prod && $product_warehouse) {
                                    if ($unit_prod->operator == '/') {
                                        $product_warehouse->qte += $prod_detail['quantity'] / ($unit_prod->operator_value ?? 1);
                                    } else {
                                        $product_warehouse->qte += $prod_detail['quantity'] * ($unit_prod->operator_value ?? 1);
                                    }
    ...
    public function show(Request $request, $id)
    {
        ...
        $purchase_data['expected_delivery_date'] = $purchase->expected_delivery_date;
    ...
    public function Purchase_pdf(Request $request, $id)
    {
        ...
        $purchase['expected_delivery_date'] = $Purchase_data->expected_delivery_date;
