...
class ProductsController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        $Filtred = $helpers->filter($products, $columns, $param, $request)
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    ...
                    $fields = ['products.name', 'products.code', 'products.note','category','brand'];
                    $parts = preg_split('~\s+~i', trim($request->search));

                    foreach ($fields as $field) {

                        $query->orWhere(function ($query) use ($parts, $field) {

                            foreach ($parts as $part) {
                                $query->where(function ($query) use ($part, $field) {
                                    ...
                                    switch($field){
                                        ...
                                        case 'products.note':
                                    $query
                                        ->where($field, 'LIKE', "%{$part}%");
                                        case 'category':
                                     $query
                                        ->orWhere(function ($query) use ($part) {
                                            return $query->whereHas('category', function ($q) use ($part) {
                                                $q->where('name', 'LIKE', "%{$part}%");
                                            });
                                        });
                                        case 'brand':
                                        $query->orWhere(function ($query) use ($part) {
                                            return $query->whereHas('brand', function ($q) use ($part) {
                                                $q->where('name', 'LIKE', "%{$part}%");
                                            });
                                        });
                                    }
        ...
        foreach ($products as $product) {
            ...
            $supplier = $product->supplier ? $product->supplier : 'N/D'; //->name
            $item['note'] = $product['note'] ? $product['note'] : null;
            ...
            // 
            $item['stock_alert'] = $product['stock_alert'] ?? 0;
            ...
            if ($product->type == 'is_single') {
                ...
                // Purchase Total
                $purchase_warehouse_total_qty = PurchaseDetail::where('product_id', $product->id)
                    // ->where('deleted_at', '=', null)
                ...
                $item['pending_purcahse_quantity'] = $purchase_warehouse_total_qty . ' ' . $product['unit']->ShortName;
    ...
    public function lowStockIndex(request $request)
    {
        ...
        $products = product_warehouse::with('warehouse', 'product', 'product.unit', 'product.category', 'product.supplier', 'product.brand', 'productVariant')
            ->addSelect('product_warehouse.warehouse_id')
            ->addSelect('product_warehouse.product_id')
            ->addSelect('product_warehouse.qte')
        ...
        $totalRows = count($Filtred->get()); //$products->count();//$Filtred->count();
        if ($perPage == "-1") {
            $perPage = $totalRows;
        }
        $products = $Filtred->offset($offSet)
        ...
        foreach ($products as $product) {
            ...
            $supplier = isset($product['supplier']) ? $product['supplier'] : 'N/D'; //->name
            $item['note'] = $product['note'] ? $product['note'] : null;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $productRules = [
                ...
                'supplier_id' => 'exists:providers,id|nullable',
            ];
            ...
            \DB::transaction(function () use ($request) {
                ...
                if ($request['type'] == 'is_single') {
                    ...
                    $Product->unit_sale_id = $request['unit_sale_id'] ? $request['unit_sale_id'] : $request['unit_id'] ?? null;
                    $Product->unit_purchase_id = $request['unit_purchase_id'] ? $request['unit_purchase_id'] : $request['unit_id'] ?? null;


                    $Product->stock_alert = $request['stock_alert'] ? $request['stock_alert'] : null;
