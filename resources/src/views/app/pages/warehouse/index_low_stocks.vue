<template>
  <div class="main-content">
    <breadcumb :page="$t('Low_Stock')" :folder="$t('Stock_List')" />
    <div v-if="isLoading" class="loading_page spinner spinner-primary mr-3"></div>
    <div v-else>
      <!-- {{ {Filter_category_Tab_index} }} -->
      <b-tabs v-model="Filter_category_Tab_index" content-class="mt-3">
        <b-tab :title="$t('All')" :active="Filter_category_Tab_index == 0">
        </b-tab>
        <!-- <p>I'm the first tab</p> -->
        <b-tab v-for="(cat) in categories" :key="cat.id" :title="cat.name" :active="Filter_category == cat.id">
          <!-- <p>I'm the first tab</p> -->
        </b-tab>
        <!-- <b-tab title="Second">
      ...
      <vue-good-table mode="remote" :columns="columns" :totalRows="totalRows" :rows="products"
        @on-page-change="onPageChange" @on-per-page-change="onPerPageChange" @on-sort-change="onSortChange"
        @on-search="onSearch" :select-options="{
      enabled: true,
      clearSelectionText: '',
      ...
      prevLabel: 'prev',
        ...
        <div slot="table-actions" class="mt-2 mb-3">
          ...
          <vue-excel-xlsx class="btn btn-sm btn-outline-danger ripple m-1" :data="products" :columns="columns"
            :file-name="'products'" :file-type="'xlsx'" :sheet-name="'products'">
            <i class="i-File-Excel"></i> EXCEL
          </vue-excel-xlsx>
          <b-button @click="Show_import_products()" size="sm" variant="info m-1"
            v-if="currentUserPermissions && currentUserPermissions.includes('product_import')">
            <i class="i-Download"></i>
            {{ $t("import_products") }}
          </b-button>
          <router-link class="btn-sm btn btn-primary btn-icon m-1"
            v-if="currentUserPermissions && currentUserPermissions.includes('products_add')" to="/app/products/store">
        ...
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <router-link v-if="currentUserPermissions && currentUserPermissions.includes('products_view')"
              v-b-tooltip.hover title="View" :to="{ name: 'detail_product', params: { id: props.row.id } }">
              <i class="i-Eye text-25 text-info"></i>
            </router-link>
            <router-link v-if="currentUserPermissions && currentUserPermissions.includes('products_edit')"
              v-b-tooltip.hover title="Edit" :to="{ name: 'edit_product', params: { id: props.row.id } }">
              <i class="i-Edit text-25 text-success"></i>
            </router-link>
            <a v-if="currentUserPermissions && currentUserPermissions.includes('products_delete')"
              @click="Remove_Product(props.row.id)" v-b-tooltip.hover title="Delete" class="cursor-pointer">
          ...
          <b-row>
            ...
            <b-col md="12">
              <b-form-group :label="$t('CodeProduct')">
                <b-form-input label="Code Product" :placeholder="$t('SearchByCode')"
            ...
            <b-col md="12">
              <b-form-group :label="$t('ProductName')">
                <b-form-input label="Name Product" :placeholder="$t('SearchByName')"
            ...
            <b-col md="12">
              <b-form-group :label="$t('Categorie')">
                <v-select :reduce="label => label.value" :placeholder="$t('Choose_Category')" v-model="Filter_category"
            ...
            <b-col md="12">
              <b-form-group :label="$t('Brand')">
                <v-select :reduce="label => label.value" :placeholder="$t('Choose_Brand')" v-model="Filter_brand"
          ...
          <b-row>
            ...
            <b-col md="12" sm="12" class="mb-3">
              <b-form-group>
                ...
                <b-form-invalid-feedback id="File-feedback" class="d-block">{{ $t('field_must_be_in_csv_format')
                  }}</b-form-invalid-feedback>
              </b-form-group>
            </b-col>

            <b-col md="6" sm="12">
              <b-button type="submit" variant="primary" :disabled="ImportProcessing" size="sm" block>{{ $t("submit")
                }}</b-button>
            ...
            <b-col md="6" sm="12">
              <a :href="'/import/exemples/import_products.csv'" class="btn btn-info btn-sm btn-block">{{
...
export default {
  ...
  watch: {
    Filter_category_Tab_index(index) {

      if (this.Filter_category_Tab_index == 0) {
        this.Filter_category = "";
      } else {
        this.Filter_category = this.categories[this.Filter_category_Tab_index - 1]?.id ?? "";
      }
  ...
  computed: {
    ...
    columns() {
      return [
        ...
        {
          label: this.$t("Stock Alert"),
          field: "stock_alert",
          tdClass: "text-left",
          thClass: "text-left"
        },
        {
          label: this.$t("Pending Purchase"),
          field: "pending_purcahse_quantity",
          tdClass: "text-left",
          thClass: "text-left"
        },
        {
