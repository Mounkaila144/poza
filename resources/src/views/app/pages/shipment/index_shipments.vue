    ...
    <b-card class="wrapper" v-if="!isLoading">
      ...
      prevLabel: 'prev',
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <div v-else-if="props.column.field == 'purchase.Ref'">
            <router-link :to="'/app/purchases/detail/' + props.row.purchase.id">
              <span class="ul-btn__text ml-1">{{ props.row.purchase.Ref }}</span>
            </router-link> <br>
            <small v-if="props.row.purchase.purchase_has_return == 'yes'"><i class="text-15 text-danger i-Back"></i></small>
          </div>
...
export default {
  ...
  methods: {
    ...
    Submit_Shipment() {
      // Accéder à l'observateur de validation par référence
      this.$refs.Create_Shipment.validate().then(success => {
        if (!success) {
          // Afficher un message si le formulaire n'est pas correctement rempli
          this.makeToast(
              "danger",
          ...
          // Si le formulaire est valide, créer ou mettre à jour
          if (!this.editmode) {
            this.Create_Shipment();  // Appel à la fonction pour créer une expédition
          } else {
            this.Update_Shipment();  // Appel à la fonction pour mettre à jour une expédition
          }
        }
      }).catch(error => {
        console.error("Error during validation:", error); // Capture des erreurs éventuelles
      });
    },
